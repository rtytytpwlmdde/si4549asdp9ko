...
class Dosen extends CI_Controller{
	...
	function peta_jadwal_kuliah(){
		...
		$data['main_view'] = 'dosen/v_peta_jadwal_kuliah';
		...
		$this->load->view('template/template_dosen',$data);
	}

	function agenda(){
		$date = date("Y-m-d");
		$data['main_view'] = 'dosen/v_list_agenda';
	...
	function peta_ruangan(){
		...
		$data['main_view'] = 'dosen/v_peta_ruangan_rutin';
		$data['mata_kuliah'] = $this->m_admin->get_data_matakuliah();
		$data['jam_kuliah'] = $this->m_admin->tampilJamKuliah()->result();
		$data['jadwal_kuliah'] = $this->m_admin->tampilJadwalKuliahToDay($day);
	...
	function peta_ruangan_non_rutin(){
		$date = date("Y-m-d");
		$data['main_view'] = 'dosen/v_peta_ruangan_non_rutin';
	...
	function tambahPeminjamanRutin(){
		...
		if($cek_rutin > 0 ){
			...
			redirect('dosen/inputPeminjamanRutin/');
		}else{ 
			if($cek_non_rutin > 0 ){
				...
				redirect('dosen/history_peminjaman/'.$id_peminjam);
			}
		}
	}
	...
	function inputPeminjamanNonRutin(){
		...
		$data['main_view'] = 'dosen/v_tambah_peminjaman_ruang_non_rutin';		
		$this->load->view('template/template_dosen',$data);
	}
	...
	function tambahPeminjamanNonRutin(){
		...
		redirect('dosen/input_ruangan_peminjaman_non_rutin/'.$id_peminjaman_non_rutin);
	}
	...
	function input_ruangan_peminjaman_non_rutin($id_peminjaman_non_rutin){
		...
		$data['main_view'] = 'dosen/v_tambah_ruangan_peminjaman_non_rutin';
		$this->load->view('template/template_dosen',$data);
	}
	...
	function hapus_ruangan_peminjaman_non_rutin($id_peminjaman,$id_ruangan){
		...
		redirect('dosen/input_ruangan_peminjaman_non_rutin/'.$id_peminjaman);
	}
	...
	function inputPeminjamanKhusus(){
		$data['mahasiswa'] = $this->m_admin->tampilMahasiswa()->result();
		$data['penyelenggara'] = $this->m_admin->tampilPenyelenggara()->result();
		$data['main_view'] = 'dosen/v_tambah_peminjaman_khusus';		
		$this->load->view('template/template_dosen',$data);
	}
	...
	function tambahPeminjamanKhusus(){
		...
		$id_peminjaman_non_rutin = $kode_tgl."".$jam_mulai_peminjaman."".$jam_selesai_peminjaman."".$penyelenggara."".$random;
			...
			$this->m_admin->tambahdata($data_pinjam,'peminjaman');
			$this->session->set_flashdata('notif', "Peminjaman berhasil dibuat, tambahkan ruangan yang akan digunakan untuk menyelesaikan proses peminjaman");
			redirect('dosen/input_ruangan_peminjaman_khusus/'.$id_peminjaman_non_rutin);
	...
	function input_ruangan_peminjaman_khusus($id_peminjaman_non_rutin){
		...
		$data['main_view'] = 'dosen/v_tambah_ruangan_peminjaman_khusus';
		$this->load->view('template/template_dosen',$data);
	}
	...
	function hapus_ruangan_peminjaman_khusus($id_peminjaman,$id_ruangan){
		...
		$this->m_admin->hapus_data($where,'detail_peminjaman_non_rutin');
		$this->session->set_flashdata('notif', "Ruangan berhasil dihapus");
		redirect('dosen/input_ruangan_peminjaman_khusus/'.$id_peminjaman);
	}
	//

	function history_peminjaman($id){
		$data['main_view'] = 'dosen/v_list_peminjaman';
	...
	function detail_peminjaman($id,$jenis){
		if($jenis == "rutin"){
			$data['main_view'] = 'dosen/v_detail_peminjaman_rutin';
			...
			redirect('dosen/input_barang_peminjaman_barang/'.$id);
		}elseif($jenis == "khusus"){
			redirect('dosen/input_ruangan_peminjaman_khusus/'.$id);
		}else{
			redirect('dosen/input_ruangan_peminjaman_non_rutin/'.$id);
		}
	}
...
//peminjaman barang
	function inputPeminjamanBarang(){
		...
		$data['penyelenggara'] = $this->m_admin->tampilPenyelenggara()->result();
		$data['main_view'] = 'dosen/v_tambah_peminjaman_barang';		
		$this->load->view('template/template_dosen',$data);
	}
	...
	function tambahPeminjamanBarang(){
		...
		$id_peminjaman_barang = $kode_tgl."".$jam_mulai_peminjaman."".$jam_selesai_peminjaman."".$penyelenggara."".$random;
			...
			redirect('dosen/input_barang_peminjaman_barang/'.$id_peminjaman_barang);
	...
	function input_barang_peminjaman_barang($id_peminjaman_barang){
		...
		$data['main_view'] = 'dosen/v_tambah_barang_peminjaman_barang';
		$this->load->view('template/template_dosen',$data);
	}
	...
	function hapus_barang_peminjaman_Barang($id_peminjaman,$id_detail){
		...
		redirect('dosen/input_barang_peminjaman_barang/'.$id_peminjaman);
	}
	...
	function peta_sarpras(){
		$date = date("Y-m-d");
		$data['main_view'] = 'dosen/v_peta_sarana_prasarana';
	...
	function filter_peta_sarpras(){
		...
		function profil(){
		$data['main_view'] = 'dosen/v_profil';
		$data['user'] = $this->m_admin->getDataProfilPegawai();   
		$this->load->view('template/template_dosen', $data);
		}
   ...
   function update_password(){
        ...
        //
        $cek = $this->m_auth->cek_login("pegawai",$data_lama)->num_rows();
		if($cek > 0){
            $this->m_auth->update_password($where,$data_baru,'pegawai');
